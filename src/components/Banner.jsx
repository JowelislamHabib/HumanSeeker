...
const Banner = () => {
  return (
    <div className="relative w-full overflow-hidden bg-zinc-50 dark:bg-zinc-950 py-24 md:py-36 transition-colors duration-300">
        ...
        <div className="w-full max-w-4xl mt-4">
          <form className="w-full flex flex-col md:flex-row items-center gap-2 p-2 rounded-2xl md:rounded-full border border-zinc-200 dark:border-zinc-800 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-md transition-all duration-300 focus-within:border-indigo-500/55 focus-within:bg-white/80 dark:focus-within:bg-zinc-900/60 focus-within:ring-4 focus-within:ring-indigo-500/10">
            ...
            <div className="w-full flex items-center gap-3 px-4 py-2.5 group">
              <RiSearchLine className="text-zinc-400 dark:text-zinc-500 w-5 h-5 shrink-0 group-focus-within:text-indigo-500 dark:group-focus-within:text-indigo-400 transition-colors duration-200" />
              <input
                type="text"
            ...
            <div className="hidden md:block h-6 w-px bg-zinc-200 dark:bg-zinc-800 shrink-0 transition-colors duration-300" />

            {/* Location Input */}
            <div className="w-full flex items-center gap-3 px-4 py-2.5 group">
              <RiMapPinLine className="text-zinc-400 dark:text-zinc-500 w-5 h-5 shrink-0 group-focus-within:text-indigo-500 dark:group-focus-within:text-indigo-400 transition-colors duration-200" />
              <input
                type="text"
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {popularCategories.map((cat) => (
              <a
                key={cat.label}
                href={cat.href}
                className="rounded-full border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 px-4 py-1.5 text-xs text-zinc-600 dark:text-zinc-300 transition-all duration-200 font-semibold cursor-pointer active:scale-95 shadow-sm"
              >
                {cat.label}
              </a>
            ))}
